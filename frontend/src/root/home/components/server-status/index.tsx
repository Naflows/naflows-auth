import { FetchServerStatus } from "./useEffect/fetch-server-status";
...
const Status = ({
  ...
  setServerStatus(FetchServerStatus().serverStatus);

  return (
    <div
      className={`nass_service__informations nass_homepage__box  ${
        serverStatus && serverStatus.status == 200
    ...
    >
      ...
      >
        ...
        <div className={`service__info__display`} style={{
          display: serverStatus && serverStatus.status == 200 ? "block" : "none"
        }}>
          <div
          ...
          >
            <div className="nass_service__system__desc__item">
              ...
              <span>Deployed on {serverStatus?.data.software.platform}</span>
            </div>
            <div className="nass_service__system__desc__item">
              <h5>System Environment</h5>
              <span>{serverStatus?.data.software.environment}</span>
            </div>
            <div className="nass_service__system__desc__item">
              <h5>System Version</h5>
              <span>{serverStatus?.data.software.version}</span>
            </div>

            <div className="nass_service__system__desc__item">
              <h5>Memory Usage</h5>
              <span>{serverStatus?.data.memory.usagePercent}%</span>
            </div>
            <div className="nass_service__system__desc__item">
              <h5>CPU Load</h5>
              <span>{serverStatus?.data.cpu.load.toFixed(2)}%</span>
            </div>
            <div className="nass_service__system__desc__item">
              <h5>CPU Cores</h5>
              <span>{serverStatus?.data.cpu.cores}</span>
            </div>
            <div className="nass_service__system__desc__item">
              <h5>Disk Usage</h5>
              <span>{serverStatus?.data.disk.usagePercent}%</span>
            </div>
          </div>
        </div>
      </div>
